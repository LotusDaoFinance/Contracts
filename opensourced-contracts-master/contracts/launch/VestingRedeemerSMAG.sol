...
contract VestingRedeemerSMAG is OwnableBase {
    ...
    struct ReceiverInfo {
        ...
        uint256 lotusAmountRedeemed;
        uint256 initialLotusAmount;
        bool isRegistered;
    }
    ...
    constructor(
        ...
        lotus = LOTUS(_lotusaddress);
        startDate = _startDate;
        periodLength = _periodLength;
        numberOfPeriods = _numberOfPeriods;
    }

    function convertLOTUSToMag(uint256 amount)
        internal
        view
    ...
    function remainingToWithdraw() public view returns (uint256 amount) {
        ...
        uint256 magAmount = convertsLOTUSToLotus(lotusAmount);
    ...
    function currentlyClaimable() public view returns (uint256) {
        ...
        if (!userInfo.isRegistered){
            return lotus.balanceOf(msg.sender).mul(percentVestedFor()).div(10000);
        }
        uint256 redeemableAmountTotal = percentVestedFor()
            .mul(userInfo.initialLotusAmount)
            .div(10000);
        if (redeemableAmountTotal < userInfo.lotusAmountRedeemed) {
            return 0;
        }
        return redeemableAmountTotal.sub(userInfo.lotusAmountRedeemed);
    }
    ...
    function claim() public {
        ...
        uint256 alreadyWithdrawn = userInfo.lotusAmountRedeemed;
        ...
        uint256 withdrawableAmount = convertsLOTUSToLotus(
            redeemableAmountFromInitial
        ).mul(index).div(10**9);
        // redeem
        lotus.redeem(msg.sender, redeemableAmountFromInitial);
        // transfer token
        IMagStaking(stakingContract).claim(address(this));
        ...
        receiverInfoMap[msg.sender] = ReceiverInfo({
            ...
            lotusAmountRedeemed: userInfo.lotusAmountRedeemed.add(
                redeemableAmountFromInitial
            ),
            initialLotusAmount: userInfo.initialLotusAmount,
            isRegistered: true
        });
